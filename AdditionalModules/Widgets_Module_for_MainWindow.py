...
class TextMaker:
    def __init__(self, row,col, window, text):
        ...
        self.text = Text(window, height=8,width = 30)

        scroll = Scrollbar(window, command=self.text.yview)
        self.text.configure(yscrollcommand=scroll.set)

        self.text.grid(row=row+1, column=col,columnspan=11, sticky="nsew")
        scroll.grid(row=row+1, column=col+12, sticky="ns")
...
class TextIOPair:
    def __init__(self, row, window):
        ...
        self.output_text_field = TextMaker(row, 13, window, "Output")
